...
function init() {
  ...
  yScale = d3
    .scaleLinear()
    .domain([0, d3.max(state.data, d => d.state_response_level)])
...
function draw() {
  ...
  const dot = svg
    .selectAll(".dot")
    .data(filteredData, d => d.year_month) // use `d.year_month` as the `key` to match between HTML and data elements
    .join(
      enter =>
